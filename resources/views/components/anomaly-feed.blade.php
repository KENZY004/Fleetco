    ...
    <div class="py-6 px-8 border-b border-white/5 bg-white/[0.02] flex justify-between items-center">
        ...
        <div class="flex items-center gap-4">
            <button 
                ...
                class="text-[9px] font-bold text-orange-500 uppercase tracking-widest hover:text-orange-400 transition-colors"
            >
                Enable Notifications
            </button>
            <div class="flex items-center gap-2 px-2.5 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-lg">
                <div class="h-1.5 w-1.5 bg-emerald-500 rounded-full" :class="isScanning ? 'animate-pulse' : ''"></div>
                ...
                <div class="p-6 hover:bg-white/[0.04] transition-colors flex flex-col gap-4 group">
                    ...
                    <div class="flex items-center gap-2 pl-15">
                        <template x-if="alert.driver?.phone_number">
                            <a :href="'tel:' + alert.driver.phone_number" class="px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-[10px] font-bold text-zinc-400 uppercase tracking-widest hover:bg-white/10 hover:text-white transition-all flex items-center gap-2">
                        ...
                        <button @click="window.dispatchEvent(new CustomEvent('inspect-alert', { detail: alert }))" class="px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-[10px] font-bold text-zinc-400 uppercase tracking-widest hover:bg-white/10 hover:text-white transition-all flex items-center gap-2">
                            <svg class="w-3 h-3 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            Inspect
                        </button>
                        <button @click="dismissAlert(alert.id)" class="px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-[10px] font-bold text-zinc-400 uppercase tracking-widest hover:bg-white/10 hover:text-white transition-all">
                            Resolve
                        </button>
                    </div>
                </div>
