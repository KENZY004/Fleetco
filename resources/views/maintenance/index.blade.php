...
@section('content')
<div class="space-y-6 md:space-y-8" x-data="{ showLogModal: false }">
    ...
    <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div class="flex items-center gap-4 md:gap-6">
            ...
            <div class="h-12 w-12 md:h-16 md:w-16 rounded-xl md:rounded-2xl bg-zinc-900 border border-white/10 flex items-center justify-center text-primary flex-shrink-0">
            ...
            <div class="min-w-0">
                <div class="text-[8px] md:text-[10px] font-bold text-zinc-500 uppercase tracking-[0.3em] mb-1 md:mb-2">Fleet_Maintenance</div>
                <h1 class="font-heading text-xl md:text-3xl font-bold text-white truncate">{{ $vehicle->name }}</h1>
                <p class="text-zinc-500 text-[10px] md:text-sm mt-0.5">Health & Service Diagnostics</p>
            </div>
        </div>

        <button @click="showLogModal = true" class="w-full lg:w-auto px-6 py-4 lg:py-3 bg-white text-black rounded-xl lg:rounded-full text-[10px] md:text-[11px] font-bold uppercase tracking-wider hover:bg-zinc-200 transition-all shadow-lg active:scale-95">
            Log New Service
        </button>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        ...
        {{-- Odometer --}}
        <div class="p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] bg-zinc-950 border border-white/5 relative overflow-hidden group">
            <div class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-4 md:mb-6">Current Mileage</div>
            <div class="text-4xl md:text-5xl font-black text-white mb-2 tracking-tighter">{{ number_format($vehicle->odometer, 1) }}</div>
            ...
            <div class="absolute bottom-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
                <svg class="w-16 h-16 md:w-24 md:h-24" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67z"/></svg>
            </div>
        </div>

        {{-- Service Status --}}
        <div class="p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] bg-zinc-950 border border-white/5 flex flex-col justify-between">
            <div>
                <div class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-4 md:mb-6">Service Health</div>
                @if($serviceDue)
                    <div class="flex items-center gap-3 text-red-500 mb-2">
                        <svg class="w-5 h-5 md:w-6 md:h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>
                        <span class="text-lg md:text-xl font-black tracking-tight uppercase">Service Due</span>
                    </div>
                @else
                    <div class="flex items-center gap-3 text-emerald-500 mb-2">
                        <svg class="w-5 h-5 md:w-6 md:h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z"/></svg>
                        <span class="text-lg md:text-xl font-black tracking-tight uppercase">Optimal</span>
                    </div>
                @endif
            </div>
        ...
        {{-- Financial Impact --}}
        <div class="p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] bg-zinc-950 border border-white/5">
            <div class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-4 md:mb-6">Lifetime Maintenance</div>
            <div class="text-3xl md:text-4xl font-black text-white mb-2 tracking-tighter">₹{{ number_format($records->sum('cost'), 0) }}</div>
            ...
            <table class="w-full text-left">
                ...
                @forelse($records as $record)
                    <tr class="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td class="px-8 py-6">
                            <div class="text-zinc-400 font-mono text-sm">{{ number_format($record->odometer_reading, 0) }} KM</div>
                        </td>
                        <td class="px-8 py-6 text-right">
                            <div class="text-white font-black">₹{{ number_format($record->cost, 0) }}</div>
                        </td>
                    </tr>
                @empty
                    <tr>
        ...
        @forelse($records as $record)
            <div class="p-6 rounded-[2rem] bg-zinc-950 border border-white/5 space-y-4">
                <div class="flex justify-between items-start">
                    <div>
                        <div class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-1">{{ $record->service_date->format('M d, Y') }}</div>
                        <div class="text-lg font-bold text-white">{{ $record->service_type }}</div>
                    </div>
                    <div class="text-lg font-black text-white">₹{{ number_format($record->cost, 0) }}</div>
                </div>
                <div class="flex justify-between items-center pt-4 border-t border-white/5">
                    <div class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Odometer</div>
                    <div class="text-xs font-mono text-zinc-400">{{ number_format($record->odometer_reading, 0) }} KM</div>
                </div>
            </div>
        @empty
            <div class="p-12 rounded-[2rem] bg-zinc-950 border border-white/5 text-center">
                <div class="text-zinc-600 italic text-sm mb-2">No records found.</div>
                <button @click="showLogModal = true" class="text-primary text-[10px] font-bold uppercase tracking-widest hover:underline">Log first service</button>
            </div>
        @endforelse
    </div>
</div>
